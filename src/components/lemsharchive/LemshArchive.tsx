import { formatDayIndex, toDayParam } from '../../lib/lemsh';
import type { PlayedDay } from '../../lib/lemshStore';
import styles from './LemshArchive.module.scss';

/*
 * Every day up to today, newest first: the ones already played carry their
 * score, the rest are still open. Nothing here reaches past today — a round
 * that has not turned over yet has no row.
 */

interface LemshArchiveProps {
...
const LemshArchive = ({ day, today, played, onPick }: LemshArchiveProps) => {
  const byDay = new Map(played.map((entry) => [entry.day, entry]));
  const days = Array.from({ length: today + 1 }, (_, index) => today - index);

  return (
    <section className={styles.archive} aria-labelledby="lemsh-arkivi">
      <h2 className={`${styles.label} sc`} id="lemsh-arkivi">
        <span>Ditët e kaluara</span>
        <span>{days.length} lëmshe</span>
      </h2>
