...
// forked from design-sync lib/bundle.mjs — add sass + css-modules loader so the
// main IIFE bundle can resolve the components' `.module.scss` imports (esbuild
// core has no .scss loader). Only the esbuild plugin set is changed; the IIFE
// header, format, exports, and CSS-sidecar contract are untouched.
...
// Compile SCSS on load, then hand the CSS to esbuild's CSS pipeline.
// `*.module.scss` → `local-css` (esbuild generates scoped names + a default
// export map, so `import styles from './x.module.scss'` works and the scoped
// CSS lands in the `_ds_bundle.css` sidecar reachable from styles.css). Plain
// `*.scss` → global `css`.
const sassModulesPlugin = {
  name: 'sass-modules',
  setup(build) {
    const compile = (path) =>
      sass.compile(path, { style: 'expanded', loadPaths: [dirname(path)] }).css;
    ...
    build.onLoad({ filter: /\.scss$/ }, (args) => {
      if (/\.module\.scss$/.test(args.path)) return undefined; // handled above
      return { contents: compile(args.path), loader: 'css', resolveDir: dirname(args.path) };
    });
  },
};
...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  const plugins = [sassModulesPlugin, reactShim];
